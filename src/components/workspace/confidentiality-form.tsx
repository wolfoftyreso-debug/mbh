...
export function ConfidentialityForm({ publicId, level, aiPolicy, portfolioPermission }: { publicId: string; level: Level; aiPolicy: Ai; portfolioPermission: Perm }) {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-xs text-ink-2">{CONFIDENTIALITY_COPY[level].short}</p>
      {open ? (
        <div className="mt-2 space-y-3 rounded-md border border-line bg-surface p-3">
          <Field label="Level"><Select value={l} onChange={(e) => { const v = e.target.value as Level; setL(v); if (v === "STRICT_CONFIDENTIAL" && ai === "AI_ALLOWED") setAi("AI_DISABLED"); }}>{(Object.keys(CONFIDENTIALITY_COPY) as Level[]).map((k) => <option key={k} value={k}>{CONFIDENTIALITY_COPY[k].label}</option>)}</Select></Field>
