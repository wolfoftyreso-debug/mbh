...
export function ConfirmButton({ label, confirmLabel, description, action, variant = "outline", size = "sm", withReason = false, reasonPlaceholder }: { label: string; confirmLabel?: string; description?: string; action: (reason: string) => Promise<ActionResult<unknown>>; variant?: "primary" | "secondary" | "outline" | "ghost" | "danger"; size?: "sm" | "md"; withReason?: boolean; reasonPlaceholder?: string }) {
  ...
  return (
    <div className="space-y-2 rounded-md border border-line bg-surface-2 p-3">
      ...
      {withReason ? <Textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder={reasonPlaceholder ?? t("common.reason")} rows={3} className="min-h-20" /> : null}
